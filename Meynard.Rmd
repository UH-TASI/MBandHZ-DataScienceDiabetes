...
temp_data <- select(diabetic_data, readmitted, DRG_1, DRG_2, DRG_3, time_in_hospital, num_procedures, num_medications, num_lab_procedures, number_inpatient, number_emergency, age)
...
nnetModel <- nnet(formula = readmitted ~.,data = train, size = 50, maxit = 1000, MaxNWts = 5000)
...
# Perform deep neural network
# Set up the formula as per your dataset
formula <- as.formula("readmitted ~ DRG_1 + DRG_2 + DRG_3 + time_in_hospital + num_procedures + num_medications + num_lab_procedures + number_inpatient + number_emergency + age")

# Specify the architecture of the neural network (adjust layers and nodes as needed)
architecture <- c(10, 50, 2)  # 10 input nodes, 50 hidden nodes in one layer, 2 output nodes

deepnetModel <- fit(formula, data = train, method = "mlp", size = architecture, linear.output = TRUE, trControl = trainControl(method = "none"))

# Print the summary of the deep neural network model
print(deepnetModel)

# Ensure all levels are present in both train and test data
prediction <- predict(deepnetModel, test)
prediction <- as.factor(prediction)
